...
try:
    ...
    # [수정 3] 3시 방향 '고용인원' 글자 삭제 (완벽 제거)
    ax2.set_ylabel('') 

    # [수정 2] 하늘색 부분(우측 하단)에 '인원 / 업체수' 가로로 배치
    # (x=1.0은 우측 끝, y=-0.12는 x축 아래, ha='right'로 오른쪽 정렬하여 삐져나가지 않게 함)
    ax2.text(1.0, -0.12, "인원 / 업체수", transform=ax2.transAxes, 
    ...
    # [수정 1] 굵은 빨간줄 위치로 제목 올리기
    # pad 대신 y 좌표를 직접 지정(y=1.28)하여 범례보다 더 위로 올림
    plt.title(f"{selected_region} 연도별 주요 실적 추이", fontsize=20, fontweight='bold', y=1.28)
